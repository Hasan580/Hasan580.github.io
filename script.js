...
(function() {
    try {
        ...
        console.log('Early theme set to:', savedTheme);
    } catch (e) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initMusicPlayer();
    initThemeToggleEnhanced(); // Use enhanced version with fallback
});
...
function initThemeToggle() {
    ...
    // Get saved theme or default to light (removing system preference auto-detection for GitHub compatibility)
...
function setTheme(theme) {
    ...
    if (themeToggle) {
        ...
        console.warn('Theme toggle button not found in setTheme');
    }
    ...
    if (themeColorMeta) {
        ...
        console.log('Meta theme-color updated');
    }
}
...
// Fallback theme mechanism for GitHub Pages
function fallbackThemeToggle() {
    console.log('Using fallback theme toggle mechanism');
    const body = document.body;
    const themeToggle = document.getElementById('themeToggle');
    
    if (!themeToggle) return;
    
    // Check if body has fallback dark class
    const isDark = body.classList.contains('theme-fallback-dark');
    
    themeToggle.addEventListener('click', function() {
        if (isDark) {
            body.classList.remove('theme-fallback-dark');
            themeToggle.innerHTML = '<i class="fas fa-moon"></i>';
            themeToggle.title = 'Switch to Dark Mode';
            localStorage.setItem('theme', 'light');
        } else {
            body.classList.add('theme-fallback-dark');
            themeToggle.innerHTML = '<i class="fas fa-sun"></i>';
            themeToggle.title = 'Switch to Light Mode';
...
// Enhanced theme initialization with fallback
function initThemeToggleEnhanced() {
    console.log('initThemeToggleEnhanced called');
    
    // Try normal theme toggle first
    try {
        initThemeToggle();
        
        // Verify it worked by checking if CSS variables are available
        const testVar = getComputedStyle(document.documentElement).getPropertyValue('--bg-primary');
        if (!testVar || testVar.trim() === '') {
            console.warn('CSS variables not available, using fallback theme');
            fallbackThemeToggle();
        }
    } catch (error) {
        console.error('Normal theme toggle failed, using fallback:', error);
        fallbackThemeToggle();
    }
}
